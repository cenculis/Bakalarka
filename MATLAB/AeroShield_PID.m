...
while(1)                % infinite loop, until "break" command is used 
    if (stepEnable)     % if next step is enable
     ...
     % Reading output value
        ...
        y = mapped(RAW, startangle, lastangle, 0.0, 100.0); % map raw value to percent 
        
     % Pick reference
        if MANUAL                               % if Manual mode is active
            PWMvalue = AeroShield.referenceRead();          % read reference from potentiometer
            r=mapped(PWMvalue, 0, 5, 0, 100);               % map value to percent
        else 
     ...
     % actuate
         ...
         PWM=mapped(coercedInput, 0, 100, 0, 5);    % remap from percent to voltage value
